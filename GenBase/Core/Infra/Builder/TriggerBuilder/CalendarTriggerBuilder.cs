...
public partial class GenSchedulerTriggerBuilder: ICalendarTriggerBuilder {
  ...
  public ICommonTriggerStep AddCalendarEntries(List<CalendarEntry> entries) {
    ...
    if(_current is not CalendarTrigger ct)
      throw new InvalidOperationException("Current trigger is not a CalendarTrigger.");

    ct.CalendarEntries.AddRange([.. entries.Select(e => { e.CalendarTriggerId = _current.Id; return e; })]);
    return this;
  }
  ...
  /// <exception cref="ArgumentException"></exception>
  /// <exception cref="InvalidOperationException"></exception>
  public ICommonTriggerStep AddCalendarEntry(CalendarEntry entry) => AddCalendarEntries([entry]);
}
